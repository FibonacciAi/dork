...
# ── Directories ──────────────────────────────────────────────────────────────
HOME = Path.home()
SETTINGS_DIR = HOME / ".dork-plus"
DATA_DIR = HOME / "Desktop" / "DorkPlus"
...
for old_dir in [HOME / ".dork", HOME / ".onyx", HOME / ".helix", HOME / ".supergrok"]:
...
def _upload_file_to_collection(filename, file_bytes, content_type, collection_id):
    ...
    req.add_header("User-Agent", "DorkPlus/1.0")
    try:
...
def openai_image_generate(prompt, model="gpt-image-1", size="1024x1024"):
...
def api_settings():
    if request.method == "GET":
        ...
        settings["xai_key_preview"] = (xai_key[:8] + "..." + xai_key[-4:]) if xai_key else ""
        settings["openai_key_set"] = bool(openai_key)
        settings["openai_key_preview"] = (openai_key[:7] + "..." + openai_key[-4:]) if openai_key else ""
        return jsonify(settings)
    else:
        data = request.json
        data.pop("xai_key", None)
...
def api_image_generate_openai():
    ...
    try:
        result = openai_image_generate(prompt, model=model, size=size)
        return jsonify({"images": [{"filename": result["filename"], "url": result["url"], "prompt": prompt, "model": model}]})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
